...
def kmeans_clustering():
    ...
    dimension = input("Enter analysis dimension (2 or 3): ").strip()
    ...
    if dim == 2:
        ...
        # Prompt for the vector direction input.
        vector_input = input("Enter vector direction for plotting (e.g., 1,1,1): ").strip()
        try:
            vector = [float(x.strip()) for x in vector_input.split(',')]
            if len(vector) != 3:
                print("Invalid input. Please enter exactly three values separated by commas. Using default vector (1,1,1).")
                vector = [1, 1, 1]
        except ValueError:
            print("Invalid input. Non-numeric value provided. Using default vector (1,1,1).")
            vector = [1, 1, 1]
        
        # Get current axis limits and calculate the center.
        xlims = ax.get_xlim()
        ylims = ax.get_ylim()
        zlims = ax.get_zlim()
        ...
        # Determine a scaling factor based on the axis ranges.
        half_range = max((xlims[1] - xlims[0]) / 2,
                         (ylims[1] - ylims[0]) / 2,
                         (zlims[1] - zlims[0]) / 2)
        
        # Compute endpoints for the vector line.
        p1 = (center[0] - vector[0] * half_range,
              center[1] - vector[1] * half_range,
              center[2] - vector[2] * half_range)
        p2 = (center[0] + vector[0] * half_range,
              center[1] + vector[1] * half_range,
              center[2] + vector[2] * half_range)
        
        ax.plot([p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]], 
                color='red', linewidth=2, label=f'Direction ({vector[0]},{vector[1]},{vector[2]})')
